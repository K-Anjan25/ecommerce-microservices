...
A process crash after SMTP acceptance but before deletion can send a duplicate. Email delivery is therefore at-least-once; production should use provider-level idempotency where available and monitor retry age/volume.
